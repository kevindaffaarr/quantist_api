...
async def get_foreign_chart(
	...
	try:
		...
		headers = {
			...
			"last_mf": wf_obj.wf_indicators['mf'].iloc[-1].astype(str),
			"last_prop": wf_obj.wf_indicators['prop'].iloc[-1].astype(str),
			"last_pricecorrel": wf_obj.wf_indicators['pricecorrel'].iloc[-1].astype(str),
			"last_mapricecorrel": wf_obj.wf_indicators['mapricecorrel'].iloc[-1].astype(str),
			"last_vwap": wf_obj.wf_indicators['vwap'].iloc[-1].astype(str),
			"last_close": wf_obj.wf_indicators['close'].iloc[-1].astype(str),
		}
...
async def get_broker_chart(
	...
	try:
		...
		headers = {
			"stockcode": wf_obj.stockcode,
			"last_date": wf_obj.wf_indicators.index[-1].strftime("%Y-%m-%d"), # type: ignore
			"last_mf": str(wf_obj.wf_indicators['mf'].iloc[-1]),
			"last_prop": str(wf_obj.wf_indicators['prop'].iloc[-1]),
			"last_pricecorrel": str(wf_obj.wf_indicators['pricecorrel'].iloc[-1]),
			"last_mapricecorrel": str(wf_obj.wf_indicators['mapricecorrel'].iloc[-1]),
			"last_vwap": str(wf_obj.wf_indicators['vwap'].iloc[-1]),
			"last_close": str(wf_obj.wf_indicators['close'].iloc[-1]),
		}
...
async def get_screener_foreign_vprofile(
	screener_vprofile_criteria: Literal[dp.ScreenerList.vprofile_inside,dp.ScreenerList.vprofile_breakout,dp.ScreenerList.vprofile_cross_down] = dp.ScreenerList.vprofile_inside,
	n_stockcodes: int = 10,
	startdate: datetime.date | None = None,
	enddate: datetime.date = datetime.date.today(),
	radar_period: int | None = None,
